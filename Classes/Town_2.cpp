...
using namespace std;
using namespace ui;
Scene* Town_2::createScene()
{
    return Town_2::create();
}
...
bool Town_2::init()
{
    ...
    //设置背景
    background = Sprite::create("town/Town2.jpeg");
    ...
    schedule(schedule_selector(Town_2::scheduleBlood), 0.1f);  //刷新函数，每隔0.1秒
    ...
    schedule(schedule_selector(Town_2::scheduleBlue), 0.1f);  //刷新函数，每隔0.1秒

    role = my_sprites::createSprite("sprites/1-4.png");
    //设置游戏主人翁
    role->setPosition(Vec2(visibleSize.width - 50, 150));
    role->setScale(2);
    this->addChild(role, 2);

    listenerKeyboard->onKeyPressed = CC_CALLBACK_2(Town_2::onKeyPressed, this);
    listenerKeyboard2->onKeyReleased = CC_CALLBACK_2(Town_2::onKeyReleased, this);
...
void Town_2::scheduleBlood(float delta) {
    auto progress = (ProgressTimer*)this->getChildByTag(5);
    progress->setPercentage((((float)role_blood) / 10000) * 100);  //这里是百分制显示
    if (progress->getPercentage() < 0) {
        this->unschedule(schedule_selector(Town_2::scheduleBlood));
    }
}

void Town_2::scheduleBlue(float delta) {
...
void Town_2::background_move(int direction, int speed) {                                                                                 //背景移动
...
void Town_2::onKeyPressed(EventKeyboard::KeyCode keyCode, Event* event)                                  //按下按键&&跳跃
{
    ...
    {
        if (background_p.x > back_size.width - visibleSize.width) {
            Director::getInstance()->pushScene(Town_1::createScene());
        }
        break;
    }
...
void Town_2::onKeyReleased(EventKeyboard::KeyCode keyCode, Event* event)                              //释放按键
{
...
void Town_2::update(float delta)                                                                                                         //左右移动
{
    ...
    if (keyMap[a])
    {
        ...
        if (role_x > role_size_w * 2) {
            ...
            if (background_x <= back_size_w - visibleSize.width) {
    ...
    if (keyMap[d])
    {
        ...
        if (role_x < visible.width - role_size_w * 2) {
            ...
            if (background_x <= visibleSize.width) {
