...
using namespace std;
using namespace ui;
Scene* TownGate::createScene()
{
    return TownGate::create();
}
...
bool TownGate::init()
{
    ...
    //设置背景
    background = Sprite::create("town/TownGate.jpeg");
    ...
    schedule(schedule_selector(TownGate::scheduleBlood), 0.1f);  //刷新函数，每隔0.1秒
    ...
    schedule(schedule_selector(TownGate::scheduleBlue), 0.1f);  //刷新函数，每隔0.1秒

    role = my_sprites::createSprite("sprites/1-4.png");
    //设置游戏主人翁
    role->setPosition(Vec2(visibleSize.width - 120, 230));
    role->setScale(2);
    this->addChild(role, 2);

    listenerKeyboard->onKeyPressed = CC_CALLBACK_2(TownGate::onKeyPressed, this);
    listenerKeyboard2->onKeyReleased = CC_CALLBACK_2(TownGate::onKeyReleased, this);
...
void TownGate::scheduleBlood(float delta) {
    auto progress = (ProgressTimer*)this->getChildByTag(5);
    progress->setPercentage((((float)role_blood) / 10000) * 100);  //这里是百分制显示
    if (progress->getPercentage() < 0) {
        this->unschedule(schedule_selector(TownGate::scheduleBlood));
    }
}

void TownGate::scheduleBlue(float delta) {
...
void TownGate::background_move(int direction, int speed) {                                                                                 //背景移动
...
void TownGate::onKeyPressed(EventKeyboard::KeyCode keyCode, Event* event)                                  //按下按键&&跳跃
{
...
void TownGate::onKeyReleased(EventKeyboard::KeyCode keyCode, Event* event)                              //释放按键
{
...
void TownGate::update(float delta)                                                                                                         //左右移动
{
    ...
    if (keyMap[d])
    {
        ...
        if (role_x < visible.width - role_size_w * 2) {
            ...
            if (background_x <= visibleSize.width + 230) {
