...
using namespace std;
using namespace ui;
Scene* Town_1::createScene()
{
    return Town_1::create();
}
...
bool Town_1::init()
{
    ...
    //设置背景
    background = Sprite::create("town/Town1.jpeg");
    ...
    schedule(schedule_selector(Town_1::scheduleBlood), 0.1f);  //刷新函数，每隔0.1秒
    ...
    schedule(schedule_selector(Town_1::scheduleBlue), 0.1f);  //刷新函数，每隔0.1秒
    ...
    listenerKeyboard->onKeyPressed = CC_CALLBACK_2(Town_1::onKeyPressed, this);
    listenerKeyboard2->onKeyReleased = CC_CALLBACK_2(Town_1::onKeyReleased, this);
...
void Town_1::scheduleBlood(float delta) {
    auto progress = (ProgressTimer*)this->getChildByTag(5);
    progress->setPercentage((((float)role_blood) / 10000) * 100);  //这里是百分制显示
    if (progress->getPercentage() < 0) {
        this->unschedule(schedule_selector(Town_1::scheduleBlood));
    }
}

void Town_1::scheduleBlue(float delta) {
...
void Town_1::background_move(int direction, int speed) {                                                                                 //背景移动
...
void Town_1::onKeyPressed(EventKeyboard::KeyCode keyCode, Event* event)                                  //按下按键&&跳跃
{
    ...
    {
        if (background_p.x > back_size.width * 1.15) {
            Director::getInstance()->pushScene(TownGate::createScene());
        }
        break;
    }
...
void Town_1::onKeyReleased(EventKeyboard::KeyCode keyCode, Event* event)                              //释放按键
{
...
void Town_1::update(float delta)                                                                                                         //左右移动
{
    ...
    if (keyMap[a])
    {
        ...
        if (role_x > role_size_w * 2) {
            ...
            if (background_x <= back_size_w * 1.15) {
