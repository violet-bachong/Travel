...
Scene* Town_5::createScene() {
    return Town_5::create();
}
...
bool Town_5::init() {
    ...
    //设置背景
    auto background = Sprite::create("town/Town5.jpeg");
...
void Town_5::menuChangeToFourCallback(Ref* pSender) {
    Director::getInstance()->pushScene(TransitionFade::create(0.5f, Town_4::createScene()));
}


void Town_5::menuChangeToSixCallback(Ref* pSender) {
...
bool Town_5::init()
{
    ...
    // 设置背景
    background = Sprite::create("town/Town5.jpeg");
    ...
    schedule(schedule_selector(Town_5::scheduleBlood), 0.1f);  //刷新函数，每隔0.1秒
    ...
    schedule(schedule_selector(Town_5::scheduleBlue), 0.1f);  //刷新函数，每隔0.1秒


    // role = Sprite::create("role.jpg");     
    role = my_sprites::createSprite("sprites/1-8.png");
    //设置游戏主人翁
    role->setPosition(Vec2( origin.x + role->getContentSize().width, 100));
    role->setScale(2);
    this->addChild(role, 2);

    listenerKeyboard->onKeyPressed = CC_CALLBACK_2(Town_5::onKeyPressed, this);
    listenerKeyboard2->onKeyReleased = CC_CALLBACK_2(Town_5::onKeyReleased, this);
...
void Town_5::scheduleBlood(float delta) {
    auto progress = (ProgressTimer*)this->getChildByTag(5);
    progress->setPercentage((((float)role_blood) / 10000) * 100);  //这里是百分制显示
    if (progress->getPercentage() < 0) {
        this->unschedule(schedule_selector(Town_5::scheduleBlood));
    }
}

void Town_5::scheduleBlue(float delta) {
...
void Town_5::background_move(int direction, int speed) {                                                                                 //背景移动
...
void Town_5::onKeyPressed(EventKeyboard::KeyCode keyCode, Event* event)                                  //按下按键&&跳跃
{
    ...
    case EventKeyboard::KeyCode::KEY_Q:
    {
        if (background_p.x + back_size.width >= window_size.width - 200) {
            Director::getInstance()->pushScene(Town_4::createScene());
        }
        break;
    }
...
void Town_5::onKeyReleased(EventKeyboard::KeyCode keyCode, Event* event)                              //释放按键
{
...
void Town_5::update(float delta)                                                                                                         //左右移动
{
